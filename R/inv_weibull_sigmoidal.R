#' Inverse-Weibull vulnerability curve
...
#' @examples
#' curve(inv_fweibull(fweibull(x, SX=30, PX=2), SX=30, PX=2), from=0, to=5)
#' @export
inv_fweibull <- function(k, SX, PX, X=50){
  X <- X[1] # when fitting; vector may be passed but X cannot actually vary.
  V <- (X-100)*log(1-X/100)
  PX * (log(k) / log(1 - X/100))^(V / (PX * SX))
}

#' Inverse-Sigmoidal vulnerability curve
